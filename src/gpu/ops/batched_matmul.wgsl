// Batched tiled matrix multiplication: C[b,M,N] = A[b,M,K] * B[b,K,N]
...
fn main(
  ...
  let aOffset = batch * params.M * params.K;
  let bOffset = batch * params.K * params.N;
  let cOffset = batch * params.M * params.N;
  ...
  for (var t = 0u; t < numTiles; t++) {
    ...
    if (row < params.M && aCol < params.K) {
      tileA[lid.y][lid.x] = a[aOffset + row * params.K + aCol];
    } else {
      tileA[lid.y][lid.x] = 0.0;
    }

    if (bRow < params.K && col < params.N) {
      tileB[lid.y][lid.x] = b[bOffset + bRow * params.N + col];
    } else {
  ...
  if (row < params.M && col < params.N) {
    c[cOffset + row * params.N + col] = sum;
  }
}
